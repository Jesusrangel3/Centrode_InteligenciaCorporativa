...
function RemolquesPage() {
  ...
  const getHistoricalData = (r: Remolque) => {
    ...
    return months.map((month, index) => {
      ...
      // Alertas críticas de Remolques
      const criticalRemolquesMaint = remolques
        .filter((r) => r.estado === "Mantenimiento" || r.estado === "Inactivo" || (r.diasTaller && r.diasTaller > 5))
        .sort((a, b) => (b.diasTaller || 0) - (a.diasTaller || 0))
        .slice(0, 5);

      const criticalRemolquesLowPerf = remolques
        .filter((r) => r.estado === "Activo" && r.utilizacion < 80)
              ...
              <div className="flex items-center justify-between border-b pb-3 border-border/80">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground mt-0.5">Remolques en mantenimiento o con taller prolongado</p>
                  </div>
                </div>
                <span className="text-[10px] bg-warning/10 text-warning border border-warning/20 px-2 py-0.5 rounded-full font-bold">
                  {remolques.filter(r => r.estado === "Mantenimiento" || r.estado === "Inactivo").length} Críticos
                </span>
              </div>

              <div className="divide-y divide-border/60">
                {criticalRemolquesMaint.length > 0 ? (
                  criticalRemolquesMaint.map((r) => (
                    <div
                      key={r.id}
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs font-semibold text-foreground truncate">{r.tipo}</p>
                          <p className="text-[9px] text-muted-foreground truncate">{r.unidadNegocio} | Placa: {r.placa}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2.5">
                        <div className="text-right">
                          ...
                          <p className="text-[8px] text-muted-foreground">Taller este mes</p>
                        </div>
                  ...
                  <p className="text-xs text-muted-foreground py-4 text-center">No hay remolques en taller o inactivos.</p>
                )}
              </div>
            </div>
              ...
              <div className="flex items-center justify-between border-b pb-3 border-border/80">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground mt-0.5">Remolques activos operando por debajo del 80%</p>
                  </div>
                </div>
                <span className="text-[10px] bg-destructive/10 text-destructive border border-destructive/20 px-2 py-0.5 rounded-full font-bold">
                  {remolques.filter(r => r.estado === "Activo" && r.utilizacion < 80).length} En Riesgo
                </span>
              </div>

              <div className="divide-y divide-border/60">
                {criticalRemolquesLowPerf.length > 0 ? (
                  criticalRemolquesLowPerf.map((r) => (
                    <div
                      key={r.id}
                      onClick={() => handleRowClick(r)}
                      className="flex items-center justify-between py-2.5 hover:bg-muted/40 px-2 rounded-lg transition-colors cursor-pointer group"
                      ...
                      <div className="flex items-center gap-4 shrink-0">
                        <div className="w-24 space-y-1">
                          <div className="flex items-center justify-between text-[9px] font-medium">
                            ...
                            <span className="text-destructive font-bold">{r.utilizacion}%</span>
                          </div>
                          <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden">
                            <div className="h-full bg-destructive" style={{ width: `${r.utilizacion}%` }} />
                          </div>
                        </div>
                  ...
                  <p className="text-xs text-muted-foreground py-4 text-center">Todos los remolques activos cumplen la meta de utilización.</p>
                )}
              </div>
            </div>
          </div>
